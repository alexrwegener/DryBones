  ...
  private ModuleFactory getModuleFactory(Object screen) {
    ...
    if (withModule != null) {
      ...
      if (constructors.length != 1) {
        throw new IllegalArgumentException(format("Module %s for screen %s should have exactly one public constructor", moduleClass.getName(), screen));
      }
      ...
      if (parameters.length > 1) {
        throw new IllegalArgumentException(format("Module %s for screen %s should have 0 or 1 parameter", moduleClass.getName(), screen));
      }
      ...
      if (parameters.length == 1) {
        ...
        if (!screenParameter.isInstance(screen)) {
          throw new IllegalArgumentException(
              format("Module %s for screen %s should have a " + "constructor parameter that is a super class of %s", moduleClass.getName(), screen,
      ...
      try {
        ...
        throw new RuntimeException(format("Failed to instantiate module %s for screen %s", moduleClass.getName(), screen), e);
      }
    }
    ...
    if (moduleFactory == null) {
      ...
      if (withModuleFactory != null) {
        ...
        try {
          ...
          throw new RuntimeException(format("Failed to instantiate module factory %s for screen %s", withModuleFactory.value().getName(), screen), e);
        }
      }
    }
